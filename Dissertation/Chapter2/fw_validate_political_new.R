...
set.seed(24519)
smp.serms <- serms.merge[sample(nrow(serms.merge[which(serms.merge$pol.var==0),]), 39000), ] #4000
...
pol <- as.data.frame(smp.serms[, c('pol.var')])
...
full.corp <- feature_selection(cont.table,
                               ...
                               alpha = 0.01,
                               rank_by_log_odds = F)
png('pol_nonpol_uni_new.png', width=15,height=12,units="in",res=100)
